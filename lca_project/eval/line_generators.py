...
class LineGeneratorHF(SpecificLineGenerator):
    ...
    def generate_line(self, datapoint: DatapointBase, use_zero_context: bool = False, attention_visualization: bool = False) -> dict[str, int]:
...
def evaluate_generation(args: GeneratorConfig):
    ...
    if isinstance(loaded_data[0], dict):
        # input_data is a list of tests (orginal length = 144)
        # each test is a dictionary containing the following keys:
        # ['repo_id', 'repo_name', 'completion_lines', 'context_dict', 'completion_dict', 'context', 'completion', 'context_len', 'model_input']
